...
class ProcessingTimeInfo:
    r"""Processing time info container class"""
    def __init__(self, start, end, units_count, previous_time_infos):
        ...
        if len(previous_time_infos):
            ...
            for time in previous_time_infos:
                if time is not None:
                    previous_work_time = max(previous_work_time,
                                             time.total_work_time)
                    self.total_work_time += previous_work_time

        self.total_work_time_per_unit = \
            int(round(self.total_work_time / float(units)))
