...
class MatrixGenerator(InputElement):
    ...
    def get_attributes(self):
        return [], \
               [Output("output")], \
               [ComboboxParameter("type", {"Rectangle": 1,
                                           "Circle": 2,
                                           "Rectangle [mono]": 3,
                                           "Circle [mono]": 4}),
    ...
    def process_inputs(self, inputs, outputs, parameters):
        ...
        if output is None:
            raise ProcessingError("Wrong matrix type")
...
class MatrixEditor(InputElement):
    ...
    def get_attributes(self):
        return [],\
               [Output("output")], \
               [ComboboxParameter("type", {"8-bit unsigned": np.uint8,
    ...
    class Button(QLabel):
        ...
        def add_value(self, delta):
            ...
            self.element.recalculate(False,
                                     False,
                                     False,
                                     force_units_recalc=True)
...
class OpenCVGetStructuringElement(InputElement):
    ...
    def get_attributes(self):
        return [], \
               [Output("output")], \
               [ComboboxParameter("Element type",
                                  values=OrderedDict(
                                      [("MORPH_RECT", cv.MORPH_RECT),
                                       ("MORPH_ELLIPSE", cv.MORPH_ELLIPSE),
                                       ("MORPH_CROSS", cv.MORPH_CROSS)])),
                IntParameter("x element size",
                             "Width of structuring element",
                             5, min_=1, max_=255, step=1),
                IntParameter("y element size", "Height of structuring element",
                             5, min_=1, max_=255, step=1), ]
    ...
    def process_inputs(self, inputs, outputs, parameters):
        ...
        element = cv.getStructuringElement(element_type,
                                           (x_element_size, y_element_size))
