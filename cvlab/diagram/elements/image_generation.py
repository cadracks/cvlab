...
class InPaint(NormalElement):
    ...
    def get_attributes(self):
        return [Input("input"), Input("mask")], \
               [Output("output")], \
               [IntParameter("radius", value=3, min_=1, max_=100),
                IntParameter("flags")]

    def process_channels(self, inputs, outputs, parameters):
        output = cv.inpaint(inputs["input"].value,
                            inputs["mask"].value,
                            parameters["radius"],
                            parameters["flags"])
...
class GetGaborKernel(InputElement):
    ...
    def process_inputs(self, inputs, outputs, parameters):
        ...
        image = cv.getGaborKernel(ksize=(ksize, ksize),
                                  sigma=sigma,
                                  theta=theta,
                                  lambd=lambda_,
                                  gamma=gamma,
