...
class InOutConnector(StyledWidget):
    def __init__(self, element, io_handle, is_input=False):
        ...
        self.workarea = None
        # workaround: the hint should be a tooltip but it is not
        # - because it can't be transparent for mouse (Qt bug)
    ...
    def dropEvent(self, e):
        source = e.source()
        if self.element is not source.element \
                and self.is_input ^ source.is_input:
    ...
    def dragMoveEvent(self, e):
        ...
        if mime in [Mime.INCOMING_CONNECTION, Mime.OUTGOING_CONNECTION]:
            if e.source().element is not self.element:
                points = [e.source().get_center_point(),
                          self.get_center_point()]
            else:
                points = [e.source().get_center_point(),
                          e.pos() + self.element.pos()]
    ...
    @pyqtSlot()
    def show_hint(self):
        # this must be in this order, because otherwise hint.width is random: /
    ...
    def update_hint_position(self):
        if self.is_input:
            ...
            delta = self.width() + 5
        self.hint.move(self.pos()
                       + self.parent().pos()
                       + QtCore.QPoint(delta, 0))
...
class PreviewsContainer(StyledWidget):
    ...
    def update(self):
        if not hasattr(self, "element"):
            # fixme: temporary hack, because there is an exception here,
            #        I do not know why!
            print("Error: ", self, " nie posiada atrybutu 'element'!")
            return

        # todo: this must be called by all slots connected to
        #       self.element.state_changed
        self.element.state_notified = False
        ...
        # todo : Is it better or better from the commentary?
        self.update_previews(state)
...
class ActionImage(QLabel):
    ...
    def __init__(self, image_preview):
        ...
        self.name = "{} {}, Output {}, Image {}".format(
            self.element.name,
            str(self.element.object_id),
    ...
    def set_image(self, arr):
        ...
        if arr is None:
            pass
            # todo : Are we sure we want it? it may delay the interface!
            # self.setPixmap(self.image_preview.default_image)
    ...
    def open_image_dialog(self):
        ...
        if self.image_dialog is None:
            ...
            self.image_dialog = \
                image_preview.manager.manager.window(self.name,
                                                     image=image,
                                                     position='cursor')
            self.image_dialog.setImage(image)
            settings = config.ConfigWrapper.get_settings()
            if bool(strtobool(settings.get_with_default(
    ...
    def close_image_dialog(self):
        if self.image_dialog is not None:
            ...
            if self.previews_container.image_dialogs_count < 0:
                ...
                # todo: report error here: reaching here would mean unexpected
                #       circumstances in counting spawning and closing dialogs
...
class ElementStatusBar(QWidget):
    ...
    def set_status(self, message=""):
        # Todo: the below is slow enough to slightly freeze the GUI;
        #       QT SIGNALS (state_changed) might be the cause
        if message != "":
    ...
    def display_processing_time(self):
        ...
        if not hasattr(self.element, "processing_time_info"):
            return
    ...
    def prepare_time_label(self):
        time_info = self.element.processing_time_info
        unit_element_time = \
            self.get_text_for_milis(time_info.work_time_per_unit)
        unit_total_time = \
            self.get_text_for_milis(time_info.total_work_time_per_unit)
    ...
    def prepare_tooltip(self):
        ...
        if time_info.work_time == time_info.work_time_per_unit:
            tt_str = "%s - Element processing time\n%s - Total processing time"
            tooltip = tt_str % (element_time, total_time)
            return tooltip
        else:
    ...
    def update(self):
        ...
        # todo: this must be called by all slots connected
        #       to self.element.state_changed
        self.element.state_notified = False
