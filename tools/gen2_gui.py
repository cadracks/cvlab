...
Compatible with OpenCV 2.3-2.4
Should be run from opencv_source/modules/python/src2/
Output classes sources are copied to the system clipboard and printed to stdout
"""
...
def copy2clipboard(what):
    r = Tk()
    r.withdraw()
    r.clipboard_clear()
    r.clipboard_append(what)
    r.destroy()
...
gen_template_rw_prop_init = Template("""
    {(char*)"${member}", (getter)pyopencv_${name}_get_${member}, (setter)pyopencv_${name}_set_${member},
    (char*)"${member}", NULL},""")
...
class FuncInfo(object):
    ...
    def gen_gui_class(self):
        """
        class OpenCVAuto_$NAME(FunctionGuiElement, OpenCVElement):
            name = "$LONGNAME"
            comment = "$DESCRIPTION"

            def get_attributes(self):
                return [Input("1", "$INPUT_1_NAME")], \
                       [Output("2", "$OUTPUT_1_NAME")], \
                       [IntParameter("1", "$PARAM_1_NAME")]

            def process_inputs(self, inputs, outputs, parameters):
                outputs["1"] = Data(cv.Canny(inputs["1"].value, parameters["1"]))
        """

        classes = ""

        for func in self.variants:
            assert isinstance(func, FuncVariant)

            try:

                name = func.name
                longname = name
                description = func.py_docstring
                ...
                for arg in func.args:
                    #arg = ArgInfo()
                    if arg.inputarg and not arg.outputarg:
                        #print "Typ parametru:", arg.tp
                        if arg.tp in ["Mat"]:
                            inputs.append(arg)
                        elif arg.tp in ["int", "double", "float", "bool", "Size", "Point"]:
                            params.append(arg)
                    elif (arg.returnarg or arg.outputarg) and not arg.inputarg and arg.tp in ["Mat"]:
                        outputs.append(arg)
                ...
                code += "class OpenCVAuto_{function}(FunctionGuiElement, OpenCVElement):\n".format(function=name)
                ...
                for arg in inputs:
                    ...
                    if arg.defval:
                        ...
                        optional = ""
                    code += "Input('{name}', '{longname}'{optional})".format(name=arg.name, longname=arg.name,
                                                                             optional=optional)
                code += "], \\ \n               ["
                ...
                for arg in outputs:
                    if is_first:
                        ...
                        code += ", "
                    code += "Output('{name}', '{longname}')".format(name=arg.name, longname=arg.name)
                code += "], \\ \n               ["
                ...
                for arg in params:
                    if is_first:
                        ...
                        code += ", "
                    if arg.tp == "Size":
                        code += "SizeParameter('{name}', '{longname}')".format(name=arg.name, longname=arg.name)
                    elif arg.tp == "Point":
                        code += "PointParameter('{name}', '{longname}')".format(name=arg.name, longname=arg.name)
                    elif arg.tp in ["double", "float", "long double"]:
                        code += "FloatParameter('{name}', '{longname}')".format(name=arg.name, longname=arg.name)
                    else:
                        code += "IntParameter('{name}', '{longname}')".format(name=arg.name, longname=arg.name)
                code += "]\n"
                code += "\n"

                code += "    def process_inputs(self, inputs, outputs, parameters):\n"
                for arg in func.args:
                    if arg in inputs:
                        code += "        {name} = inputs['{name}'].value\n".format(name=arg.name)
                    elif arg in params:
                        code += "        {name} = parameters['{name}']\n".format(name=arg.name)
                ...
                for arg in outputs:
                    if is_first:
                        ...
                        code += ", "
                    code += "{name}".format(name=arg.name)
                code += " = "
                code += "cv2.{funcname}(".format(funcname=func.name)
                is_first = True
                for arg in func.args:
                    if arg in inputs or arg in params:
                        if is_first:
                            is_first = False
                        else:
                            code += ", "
                        code += "{name}={name}".format(name=arg.name)
                    elif not arg.returnarg:
                        if not arg.defval:
                            if is_first:
                                is_first = False
                            else:
                                code += ", "
                            code += "{name}=None".format(name=arg.name)
                            #else:
                            #    val = arg.defval.replace("::", ".").replace("new ", "")
                            #    code += "{name}={default}".format(name=arg.name, default=val)
                code += ")\n"
                for arg in outputs:
                    code += "        outputs['{name}'] = Data({name})\n".format(name=arg.name)
                code += "\n\n"

                classes += code

            except:
                classes += "#ERROR in function {function}\n".format(function=func.name)

        return classes


    def gen_code(self, all_classes):
        if self.name in ["Canny", "blur"]:
            pass
        ...
        for v in self.variants:
            ...
            all_code_variants.append(gen_template_func_body.substitute(code_decl=code_decl,
                                                                       code_parse=code_parse, code_fcall=code_fcall,
                                                                       code_ret=code_ret))
...
class PythonWrapperGenerator(object):
    ...
    def gen(self, srcfiles, output_path):
        ...
        for name, classinfo in classlist:
            if classinfo.ismap:
                ...
                self.code_types.write(templ.substitute(name=name, wname=classinfo.wname, cname=classinfo.cname,
                                                       cname1=(
                                                           "cv::Algorithm" if classinfo.isalgorithm else classinfo
                                                           .cname)))
        ...
        classes = ""
        for name, func in funclist:
            classes += func.gen_gui_class()
        print classes
        copy2clipboard(classes)

        # That's it. Now save all the files
        #self.save(output_path, "opencv_generated", code)
